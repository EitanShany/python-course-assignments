# Day08 - Cancer Target Antibody Search
...
A new FastAPI web app is included in this copy as `web_antibody_search.py`,
using the same business logic as the command-line code and adding input
validation to prevent unsafe query patterns.
...
```bash
pip install -r Day08/requirements.txt
```
...
```bash
python Day08/antibody_search.py --target PD1 --limit 10
```
...
```bash
python Day08/antibody_search.py --target EGFR --limit 20 --output egfr_antibodies.xlsx
```

Optional species/type filtering:

```bash
python Day08/antibody_search.py --target PD1 --species Human,Humanized
```
...
```bash
python Day08/antibody_search.py --target 5T4 --input-file previous_results.xlsx
```

CSV input files are also supported:

```bash
python Day08/antibody_search.py --target EPCAM --input-file previous_results.csv
```

## Web App

Run the FastAPI web application with Uvicorn:

```bash
python -m uvicorn Day08.web_antibody_search:app --reload
```

The web app uses the same search logic as the command-line version and provides:

- a safe target query field with input validation,
- species/type selection via user-friendly checkboxes,
- a live HTML search page,
- a JSON API endpoint at `/api/search`,
- downloads for CSV and Excel output saved locally to `Day08/output/`.
...
```bash
python Day08/gui_antibody_search.py
```
